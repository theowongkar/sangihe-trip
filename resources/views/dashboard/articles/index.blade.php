<x-app-layout>
    ...
    {{-- Judul Halaman --}}
    <x-slot name="title">Data Artikel</x-slot>
    ...
    <section class="space-y-2">
        ...
        <div class="bg-gray-50 rounded-lg border border-gray-300 shadow">
            <div class="p-2 space-y-2">
                ...
                <div x-data="{ openCreate: false }" class="flex flex-col lg:flex-row items-center justify-between gap-4">
                    {{-- Tombol Tambah --}}
                    <x-buttons.primary-button href="{{ route('dashboard.article.create') }}"
                        class="w-full lg:w-auto text-center bg-green-600 hover:bg-green-700 whitespace-nowrap">
                        Tambah Artikel
                    </x-buttons.primary-button>

                    {{-- Form Filter & Search --}}
                    <form method="GET" action="{{ route('dashboard.article.index') }}"
                        ...
                        <div class="relative">
                            ...
                            <div x-show="openFilter === 'status'" @click.away="openFilter = ''" x-transition
                                ...
                                <select name="status" x-on:change="$root.submit()"
                                    ...
                                    <option value="Terbit" {{ request('status') == 'Terbit' ? 'selected' : '' }}>Terbit
                                    </option>
                                    <option value="Arsip" {{ request('status') == 'Arsip' ? 'selected' : '' }}>Arsip
                                    </option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="w-full lg:w-80">
                            <x-forms.input type="text" name="search"
                                placeholder="Cari penerbit atau judul artikel..." autocomplete="off"
        ...
        <div class="bg-white rounded-lg border border-gray-300 shadow overflow-x-auto">
            <table class="min-w-full text-sm">
                <thead class="bg-[#115e6d] text-gray-50">
                    <tr>
                        ...
                        <th class="p-2 font-normal text-left border-r border-gray-600">Penulis</th>
                ...
                <tbody class="divide-y divide-gray-300">
                    @forelse($articles as $article)
                        <tr class="hover:bg-blue-50">
                            ...
                            <td class="p-2 border-r border-gray-300 whitespace-nowrap">{{ $article->title }}
                            </td>
                            ...
                            <td class="p-2 text-center border-r border-gray-300">
                                @php
                                    ...
                                    $colors = [
                                        ...
                                        'Arsip' => 'bg-gray-200 text-gray-800 border border-gray-400',
                                    ];
                                @endphp
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="p-4 text-center text-gray-500">Tidak ada data artikel
                            </td>
                        </tr>
                    @endforelse
                </tbody>
